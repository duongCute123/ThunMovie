...
import { addFavourite as addFavouriteAction } from "../../../store/favorites"; // đổi tên
...
const DetailPage = () => {
    ...
    useEffect(() => {
        if (detais?.movie?.episodes && detais.movie.episodes.length > 0) {
            ...
            setCurrent(tap1);
            setVietSub(detais.movie.episodes[0]);
            setThuyetMinh(detais.movie.episodes[1]);
        }
    }, [detais]);
    ...
    if (detais?.error) return <FallBack error={detais?.error?.message} />;
    if (detais.loading) {
        return (
    ...
    if (!detais) return <FallBack error={detais.payload.message} />;

    const handleAddFavourite = (details) => { // đổi tên hàm
        dispatch(addFavouriteAction(details)); // gọi action creator
    ...
    return (
        <div className="">
            ...
            <div className="bg-cover w-full aspect-video relative bg-center lg:max-h-[800px]" style={{ backgroundImage: `url(${detais?.movie?.movie?.thumb_url})` }}>
                <div className="inset-0 bg-black/90 px-4 pb-10 pt-24 flex items-center lg:absolute">
                    <div className="w-full max-w-7xl mx-auto flex flex-col items-center gap-8 md:flex-row">
                        ...
                        <div className="w-full gap-4">
                            ...
                            <p className="text-white mt-4">{detais?.movie?.movie?.content.replace("&quot;", "")}</p>
                            <div className="mt-8 flex gap-8 items-center rounded-full border-2 p-1 lg:w-[400px] h-16">
                                ...
                                <ModalSocials visible={showModal} closed={handleCloseModal} />
                                <div className="flex text-white gap-8 items-center">
                                    ...
                                    <button className="rounded-full border-2 border-yellow-500 hover:bg-yellow-500 hover:text-black w-28 mx-auto flex gap-2 justify-center h-10 text-center items-center font-bold" onClick={() => handleAddFavourite(detais)}><GiSelfLove />Yêu thích</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className={`text-white mx-10 ${vietsub.server_data?.length > 60 ? 'h-[400px] overflow-y-auto' : "overflow-y-auto"}`}>
                <h1 className="mt-6 my-4 font-semibold text-xl text-yellow-400 border-b-4 border-yellow-400 inline-block">Vietsub:</h1>
                <div className="grid grid-cols-6 gap-2 lg:grid-cols-12">
                    {vietsub?.server_data && vietsub?.server_data.map((episodes, idx) => (
                        <div className={`items-center hover:bg-yellow-400 p-1.5 hover:text-black ${current?.filename === episodes.filename ? 'bg-yellow-400' : 'bg-black/95'} text-center rounded-xl`} key={idx}>
                            <button onClick={() => scrollToVideo(episodes)}>{episodes.name.replace("Tập ", "")}</button>
                        </div>
                    ))}
                </div>
                {thuyetminh && (
                    <>
                        <h1 className="mt-6 my-4 font-semibold text-xl text-yellow-400 border-b-4 border-yellow-400 inline-block">Thuyết Minh :</h1>
                        <div className="grid grid-cols-6 gap-2 lg:grid-cols-12">
                            {thuyetminh?.server_data && thuyetminh?.server_data.map((thuyetminh, idx) => (
                                <div className={`items-center hover:bg-yellow-400 p-1.5 hover:text-black ${current?.filename === thuyetminh.filename ? 'bg-yellow-400' : 'bg-black/95'} text-center rounded-xl`} key={idx}>
                                    <button onClick={() => scrollToVideo(thuyetminh)}>{thuyetminh.name.replace("Tập ", "")}</button>
                                </div>
                            ))}
                        </div>
                    </>
